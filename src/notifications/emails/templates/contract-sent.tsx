 ...
 * 
 * React Email compatible template for contract sent notifications.
 * This can be used with @react-email/components or rendered to HTML.
 */

import React from 'react'

export interface ContractSentEmailProps {
  firstName: string
  lastName: string
...
/**
 * Generate contract sent email content
 * 
 * Returns HTML and text versions of the email.
 * This is compatible with React Email but can also be used standalone.
 */
export function generateContractSentEmail(
  props: ContractSentEmailProps
  ...
  const contractNumberText = contractNumber
    ? isFrench
      ? `Numéro de contrat : ${contractNumber}`
      : `Contract Number: ${contractNumber}`
    : ''

  const loanAmountText = loanAmount
    ? isFrench
      ? `Montant du prêt : ${new Intl.NumberFormat('fr-CA', {
          style: 'currency',
          currency: 'CAD',
        }).format(loanAmount)}`
      : `Loan Amount: ${new Intl.NumberFormat('en-CA', {
          style: 'currency',
          currency: 'CAD',
        }).format(loanAmount)}`
    : ''
  ...
  const expiresText = expiresAt
    ? isFrench
      ? `<div style="background:#fff3cd; border-left:4px solid #ffc107; padding:12px; margin:16px 0; border-radius:4px;"><p style="margin:0; font-size:13px; color:#856404;">⚠️ <strong>Important:</strong> Ce contrat expire le ${new Date(
          expiresAt
        ).toLocaleDateString('fr-CA', {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
        })}. Veuillez le signer avant cette date.</p></div>`
      : `<div style="background:#fff3cd; border-left:4px solid #ffc107; padding:12px; margin:16px 0; border-radius:4px;"><p style="margin:0; font-size:13px; color:#856404;">⚠️ <strong>Important:</strong> This contract expires on ${new Date(
          expiresAt
        ).toLocaleDateString('en-CA', {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
        })}. Please sign it before this date.</p></div>`
    : ''
  ...
  <body>
    <div class="wrapper">
      <div class="container">
        ...
        <div class="content">
          <p class="h1">${isFrench ? `Bonjour ${firstName},` : `Hi ${firstName},`}</p>
          <p class="p">${isFrench
          ...
          <div class="info-box">
            ${contractNumberText
              ? `<div class="info-item"><span class="info-label">${contractNumberText.split(':')[0]}:</span> ${contractNumberText.split(':')[1]?.trim() || ''}</div>`
              : ''}
            ${loanAmountText
              ? `<div class="info-item"><span class="info-label">${loanAmountText.split(':')[0]}:</span> ${loanAmountText.split(':')[1]?.trim() || ''}</div>`
              : ''}
          </div>
          `
            : ''}
          ...
          <div class="button-wrap">
            <a class="button" href="${dashboardUrl}">${isFrench
              ? 'Se connecter et signer le contrat'
              : 'Log In and Sign Contract'}</a>
          </div>
          ...
          <p class="p" style="text-align:center; margin-top:4px;"><span class="code">${dashboardUrl}</span></p>
        ...
        <div class="footer">
          <div>© ${new Date().getFullYear()} Flash-Loan. ${isFrench
...
${expiresAt
  ? `⚠️ Important: Ce contrat expire le ${new Date(expiresAt).toLocaleDateString('fr-CA', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    })}. Veuillez le signer avant cette date.`
  : ''}
...
${expiresAt
  ? `⚠️ Important: This contract expires on ${new Date(expiresAt).toLocaleDateString('en-CA', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    })}. Please sign it before this date.`
  : ''}
