...
export default function LoanSummaryTable({
  ...
  const handleSyncLoan = async () => {
    if (!loan?.id) return

    setSyncingLoan(true)
    try {
      const response = await fetch(`/api/admin/loans/${loan.id}/sync`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        }
      ...
      if (!response.ok) {
        const error = await response.json()
        throw new Error(error.error || 'Failed to sync loan')
      }

      const result = await response.json()
      
      // Simple message with summary
      const message = result.message ||
        `Sync completed: ${result.result?.created || 0} created, ${result.result?.failed || 0} failed`
      ...
      alert(`Failed to sync loan: ${error.message || 'Unknown error'}`)
    } finally {
      setSyncingLoan(false)
    }
  }
      ...
      <div className='border-b border-gray-200 bg-gradient-to-r from-indigo-50 to-purple-50 px-4 py-2'>
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center gap-2'>
            <button
              onClick={handleSyncLoan}
              disabled={syncingLoan}
              className='flex items-center gap-1.5 rounded-md bg-indigo-600 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed'
              title='Sync loan payments to ZumRails'
            >
              {syncingLoan ? (
                <>
                  <svg
